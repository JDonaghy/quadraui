...
impl Grid {
    ...
    pub fn col_wrap(&mut self, width: u16, wrap: bool) {
        if self.pos.col > self.size.cols - width {
            ...
            // PATCHED(vt100-0.15.2 #377): drawing_row_mut() can return None
            // for prev_pos.row when a scroll shifts rows out of the live
            // grid. Skip the wrap flag rather than panic.
