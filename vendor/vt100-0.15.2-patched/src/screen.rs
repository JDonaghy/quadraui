...
impl Screen {
    fn text(&mut self, c: char) {
        ...
        if width == 0 {
            ...
            // PATCHED(vt100-0.15.2 #377): drawing_cell() can return None for
            // wide-char continuation cells at a column boundary; treat None as
            // not-wide rather than panicking.
            if self
                .grid()
                .drawing_cell(pos)
                .map_or(false, |c| c.is_wide_continuation())
            {
                let prev_cell = self
                    .grid_mut()
                    .drawing_cell_mut(crate::grid::Pos {
            ...
            if width > 1 {
                let pos = self.grid().pos();
                // PATCHED(vt100-0.15.2 #377): drawing_cell() can return None
                // for wide-char continuation cells; treat None as not-wide.
                if self
                    .grid()
                    .drawing_cell(pos)
                    .map_or(false, |c| c.is_wide())
                {
                    ...
                    if next_next_pos.col == size.cols - 1 {
                        // PATCHED(vt100-0.15.2 #377): drawing_row_mut can
                        // also return None; skip the wrap flag rather than
                        // panic.
                        if let Some(row) =
