...
mod tests {
    ...
    fn gtk_wheel_down_scrolls_tree_toward_later_rows_traditional() {
        ...
        let outcome = tc.handle(&ev, &mut RecordingBackend::new(), rect);
    ...
    fn set_show_scrollbar_false_suppresses_scrollbar() {
        ...
        let (tree_rect, sb_rect) = tc.split_rect(&RecordingBackend::new(), rect);
    ...
    fn default_scrollbar_width_uses_line_height() {
        ...
        let (tree_rect, sb_rect) = tc.split_rect(&RecordingBackend::new(), rect);
        let sb = sb_rect.expect("scrollbar should be present");
        // RecordingBackend::new().line_height() = 1.0
    ...
    /// and selects the row. RecordingBackend's tree_layout now computes chevron
    ...
    fn chevron_click_emits_row_toggle_expand() {
        ...
        // RecordingBackend: line_height=1, indent=2, chevron='▾'(1ch)+1 → chevron_end_x=2
