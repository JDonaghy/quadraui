...
impl<A: AppLogic> GtkDriver<A> {
    ...
    /// Pixel coordinates of tab `tab_idx`'s center in the tab bar `bar`,
    /// for [`Self::click`] to land on that specific tab (quadraui#594).
    ///
    ...
    /// resolves against the [`crate::TabBarLayout`] `GtkBackend::draw_tab_bar`
    ...
    /// Pixel coordinates of tab `tab_idx`'s close-button center in the tab
...
mod tests {
    ...
    const TAB_BAR_W: i32 = 300;
    const TAB_BAR_H: i32 = 30;

    /// Three closable tabs on a wide bar. `handle()` hit-tests raw
    /// `MouseDown` clicks against the *live*, non-painting
    /// [`crate::Backend::tab_bar_layout`] query (mirrors the "cached
    /// layout hit-test pattern" [`ToggleStatusBarApp`] above uses) — close
    /// regions win over tab bodies, matching `TabBarLayout::hit_regions`'
    /// close-before-body ordering. Exercises `GtkDriver::tab_center`/
    /// `tab_close_center` end-to-end: the driver locates a specific tab's
    /// click target with no hardcoded coordinates, and the resulting
    /// click reaches the app through the ordinary event path.
    ...
    fn tab_center_click_activates_target_tab() {
        ...
        let (x, y) = driver
            .tab_center(&bar_id, 1)
            .expect("tab 1 should be visible on a wide bar with 3 short tabs");
        driver.click(x, y);
    ...
    fn tab_close_center_none_when_tab_not_closable() {
        ...
        impl AppLogic for NonClosableTabBarApp {
            ...
            fn render(&self, backend: &mut dyn Backend, _area: ()) {
                backend.draw_tab_bar(
                    Rect::new(0.0, 0.0, TAB_BAR_W as f32, TAB_BAR_H as f32),
                    &TabBar {
                        id: WidgetId::new("tabs"),
                        tabs: vec![TabItem {
                            label: "a".to_string(),
        ...
        let driver = GtkDriver::new(NonClosableTabBarApp, TAB_BAR_W, TAB_BAR_H);
        let bar_id = WidgetId::new("tabs");
    ...
    fn tab_center_none_when_scrolled_out_of_view() {
        ...
        impl AppLogic for ScrolledTabBarApp {
            ...
            fn render(&self, backend: &mut dyn Backend, _area: ()) {
                backend.draw_tab_bar(
                    // Tab 0's 80-char label alone vastly exceeds this
                    // width, so the full set can never fit — guarantees
                    // the "everything fits" layout branch (which would
                    // ignore `scroll_offset` and reset it to 0) never
                    // triggers, regardless of the host's exact font
                    // metrics. Tabs 1/2 are short, so once `scroll_offset`
                    // skips tab 0 they comfortably fit inside this same
                    // width.
                    Rect::new(0.0, 0.0, TAB_BAR_W as f32, TAB_BAR_H as f32),
                    &TabBar {
                        id: WidgetId::new("tabs"),
                        ...
                        // offset directly when the full set doesn't fit
                        // (no scroll arrows computed internally here).
                        scroll_offset: 1,
                        right_segments: vec![],
                        active_accent: None,
                        show_tab_close: true,
        ...
        let driver = GtkDriver::new(ScrolledTabBarApp, TAB_BAR_W, TAB_BAR_H);
        let bar_id = WidgetId::new("tabs");
